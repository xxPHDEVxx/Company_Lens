...
from src.features.company_scraper.schema.raw_establishment import (
    RawEstablishmentUnit,
)
...
class AnnualAccountData(BaseModel):
    ...
    employees: Optional[float] = Field(
        0, description="The number of employees in full-time equivalents (FTE)."
    )
    previous_year_revenue: Optional[float] = Field(
        0, description="Revenue from the previous year."
    )
    total_asset: Optional[float] = Field(
        0, description="The total balance sheet assets, extracted from field '10/49'."
    )
...
class EstablishmentUnitData(BaseModel):
    ...
    units: Optional[List[RawEstablishmentUnit]] = Field(
...
class LegalDataProcess(BaseModel):
    ...
    """
    financial_data_and_size: Optional[AnnualAccountData]
    establishment_units_list: Optional[List[RawEstablishmentUnit]]
